...
} from "../constants/categoriesNames";
...
describe("Favorite units", () => {
  ...
  it('"Обрані" icon functionality', function () {
    ...
    productsPage.cardWrappers.then((cards) => {
      ...
      productsPage
        .getFavouriteButtonsPath(cy.wrap(cards[0]))
        .should("not.have.attr", "fill", "#F73859");
    });
  });
  ...
  it('"Всі категорії" dropdown menu functionality', function () {
    ...
    for (const categoryName of categoriesDropdownListNames) {
      ...
      unitsPage
        .selectCategoryItemByName(categoryName)
        .should("be.visible")
        .click();
      unitsPage.unitCard.then((cards) => {
        ...
        switch (categoryName) {
          case "Комунальна техніка": {
            unitsPage
              ...
              .then((text) => {
                expect(
                  municipalEquipmentCategoryNames.some((category) =>
          ...
          case "Складська техніка": {
            unitsPage
              ...
              .then((text) => {
                expect(
                  warehouseEquipmentCategoryNames.some((category) =>
                    text.includes(category)
                  )
                ).to.be.true;
              });
