...
describe("Favorite units", () => {
  ...
  it('TC-302 "Обрані" icon functionality', function () {
    ...
    productsPage.cardWrappers.then((cards) => {
      ...
      productsPage
        .getFavouriteButtonsPath(cy.wrap(cards[0]))
        .should("have.attr", "fill", Colors.CRIMSON_COLOR);
    });
    ...
    productsPage.cardWrappers.then((cards) => {
      ...
      productsPage
        .getFavouriteButtonsPath(cy.wrap(cards[0]))
        .should("not.have.attr", "fill", Colors.CRIMSON_COLOR);
    });
  });
  ...
  it('TC-305 "Пошук по назві" search field functionality', function () {
    ...
    unitApi.getUnits(pageNumber).then((units) => {
      ...
      while (favouriteUnits.size < 4) {
        favouriteUnits.add(
          randomValue.selectRandomValueFromArray(units.body.results)
        );
      }
  ...
  it('TC-315 "Всі категорії" dropdown menu functionality', function () {
    ...
    unitApi.getCategories().then((response) => {
      ...
      const firstLevelCategories = response.body.filter(
        (category) => category.parent === null
      );
      for (let i = 0; i < firstLevelCategories.length; ++i) {
        const secondLevelCategories = response.body.filter(
          (category) => category.parent === firstLevelCategories[i].id
        );
        const thirdLevelCategories = response.body.filter(
          (category) =>
            secondLevelCategories.some(
              (secondLevel) => secondLevel.id === category.parent
            ) && category.level === 3
        );
        for (let j = 0; j < 2; ++j) {
          const randomCategory =
            randomValue.selectRandomValueFromArray(thirdLevelCategories);
          unitApi.createUnit(undefined, randomCategory.id).then((result) => {
    ...
    for (const categoryName of categories.categoriesDropdownListNames) {
      ...
      unitsPage.unitCard.then((cards) => {
        ...
        unitsPage
          .getUnitCardCategory(cy.wrap(cards).first())
          .invoke("text")
          .then((text) => {
            switch (categoryName) {
              case "Комунальна техніка": {
                expect(
                  categories.municipalEquipmentCategoryNames.some((category) =>
              ...
              case "Складська техніка": {
                expect(
                  categories.warehouseEquipmentCategoryNames.some((category) =>
                    text.includes(category)
                  )
                ).to.be.true;
                break;
              ...
              case "Будівельна техніка": {
                expect(
                  categories.constructionEquipmentCategoryNames.some(
  ...
  it('TC-303 "Очистити список" button functionality 2', function () {
    ...
    productsPage.cardWrappers.then((cards) => {
      for (let i = 0; i < 2; ++i) {
        ...
        productsPage
          .getFavouriteButtonsPath(cy.wrap(cards[i]))
          .should("have.attr", "fill", Colors.CRIMSON_COLOR);
        productsPage
          .getCardName(cy.wrap(cards[i]))
          .invoke("text")
          .then((text) => {
      ...
      unitsPage.unitCard.then((units) => {
        ...
        for (let i = 0; i < units.length - 1; ++i) {
          switch (sortName) {
            case " по назві": {
              unitsPage
                .getUnitCardName(cy.wrap(units[i]))
                .invoke("text")
                .then((firstUnitName) => {
                  unitsPage
                    .getUnitCardName(cy.wrap(units[i + 1]))
                    .invoke("text")
                    .then((secondUnitName) => {
                      expect(
                        firstUnitName.localeCompare(secondUnitName)
                      ).to.be.at.most(0);
                    });
                });
              break;
            ...
            case "по даті створення": {
              unitsPage
                .getUnitCardCreationDate(cy.wrap(units[i]))
                .invoke("text")
                .then((firstUnitDate) => {
                  unitsPage
                    .getUnitCardCreationDate(cy.wrap(units[i + 1]))
                    .invoke("text")
                    .then((secondUnitDate) => {
                      const firstDate = new Date(
                        firstUnitDate.split(".").reverse().join("-")
                      );
                      const secondDate = new Date(
                        secondUnitDate.split(".").reverse().join("-")
                      );
                      cy.wrap(firstDate.getTime()).should(
                        "be.at.least",
                        secondDate.getTime()
                      );
                    });
  ...
  it('TC-746 Check the "Будівельна техніка" menu functionality', function () {
    ...
    let unitCount: number;
    const constructionEquipmentCategory =
      categories.categoriesDropdownListNames[0];
    ...
    unitsPage.categoriesDropdownList.should("be.visible").click();
    unitsPage
      .selectListItemByName(constructionEquipmentCategory)
      .should("be.visible")
      .click();
    unitsPage.unitCard.then((cards) => {
      cy.wrap(cards)
      ...
      unitsPage
        ...
        .then((text) => {
          expect(
            categories.constructionEquipmentCategoryNames.some((category) =>
              text.includes(category)
            )
          ).to.be.true;
        });
  ...
  it('TC-747 Check the "Комунальна техніка" menu functionality', function () {
    ...
    let unitCount: number;
    const municipalEquipmentCategory =
      categories.categoriesDropdownListNames[1];
    const allCategories = categories.categoriesDropdownListNames[3];
    unitsPage.unitCard.then((cards) => {
      unitCount = cards.length;
    });
    ...
    unitsPage.categoriesDropdownList.should("be.visible").click();
    unitsPage
      .selectListItemByName(municipalEquipmentCategory)
      .should("be.visible")
      .click();
    unitsPage.unitCard.then((cards) => {
      cy.wrap(cards)
  ...
  it('TC-748 Check the "Складська техніка" menu functionality', function () {
    ...
    let unitCount: number;
    const warehouseEquipmentCategory =
      categories.categoriesDropdownListNames[2];
    const allCategories = categories.categoriesDropdownListNames[3];
    unitsPage.unitCard.then((cards) => {
      unitCount = cards.length;
    });
    ...
    unitsPage.categoriesDropdownList.should("be.visible").click();
    unitsPage
      .selectListItemByName(warehouseEquipmentCategory)
      .should("be.visible")
      .click();
    unitsPage.unitCard.then((cards) => {
      cy.wrap(cards)
